...
# Function to apply a simple moving average to smooth the loss curves
def moving_average(data, window_size=10):
    return np.convolve(data, np.ones(window_size) / window_size, mode='valid')

# Adjusted plotting for clarity
def plot_smoothed_validation_loss(results, selected_lr=0.01, window_size=10):
    plt.figure(figsize=(18, 12))
    
    # Organize subplots based on regularization types
    fig, axes = plt.subplots(1, 3, figsize=(20, 8), sharey=True)  # 3 subplots for none, l1, l2 regularization
    regularization_types = ['none', 'l1', 'l2']
    
    for i, reg_type in enumerate(regularization_types):
        ax = axes[i]
        for key, (train_loss, valid_loss, train_acc, valid_acc) in results.items():
            # Filter configurations with the desired learning rate and regularization type
            if f'lr={selected_lr}' in key and f'{reg_type}_reg' in key:
                # Smooth the validation loss with moving average
                smoothed_loss = moving_average(valid_loss, window_size)
                ax.plot(smoothed_loss, label=key, alpha=0.7)  # Set alpha for transparency
        
        # Set title for each subplot
        ax.set_title(f'Validation Loss with {reg_type.upper()} Regularization')
        ax.set_xlabel('Iterations')
        ax.set_ylabel('Validation Loss')
        ax.legend(loc='upper right', fontsize='small')
    
    # Set overall title and layout adjustments
    plt.suptitle(f'Validation Loss with Early Stopping (Learning Rate = {selected_lr}, Smoothed)', fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # Adjust layout to fit the title
    plt.show()

# Assuming your 'results' dictionary has been populated with the data you want to plot
# For example, calling the function with a learning rate of 0.01 and smoothing window of 10
plot_smoothed_validation_loss(results, selected_lr=0.01, window_size=10)
